...
create index participant_sessions_lookup on public.participant_sessions(token_hash) where revoked_at is null;
create index participant_sessions_participant on public.participant_sessions(participant_id, expires_at desc);
...
create function public.select_participant_program(p_participant_id uuid, p_program_id uuid)
...
create function public.remove_participant_program(p_participant_id uuid, p_program_id uuid)
returns void language plpgsql security definer set search_path='' as $$
declare v_row public.participant_programs; v_open timestamptz; v_close timestamptz;
begin
  select program_selection_open_at,program_selection_close_at into v_open,v_close from public.app_settings where id=true;
  if v_open is not null and now()<v_open then raise exception 'SELECTION_NOT_OPEN'; end if;
  if v_close is not null and now()>v_close then raise exception 'SELECTION_CLOSED'; end if;
  select * into v_row from public.participant_programs where participant_id=p_participant_id and program_id=p_program_id for update;
