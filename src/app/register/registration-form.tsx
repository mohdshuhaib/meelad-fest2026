...
export function RegistrationForm(){
  ...
  async function submit(data:RegistrationInput){
    ...
    try {
      ...
      if(!res.ok){setServerError(result.message??"Registration could not be completed.");return}
      sessionStorage.setItem(`registration:${result.token}`,JSON.stringify(result));
      router.push(`/register/success?token=${result.token}`);
    } catch(error) {
      setServerError("Network error: Could not reach the server. Please check your internet connection and try again.");
    }
  }
  ...
  return <form onSubmit={handleSubmit(submit)} className="space-y-7" noValidate>
    ...
    <div className="grid gap-6 sm:grid-cols-2"><label className={labelClass}>WhatsApp number<input {...register("whatsappNumber")} inputMode="tel" autoComplete="tel" maxLength={16} placeholder="+971501234567" className={inputClass}/>{field("whatsappNumber")&&<Error text={field("whatsappNumber")!}/>}</label><label className={labelClass}>Phone number<input {...register("phoneNumber")} inputMode="tel" autoComplete="tel" maxLength={16} placeholder="+971501234567" className={inputClass}/>{field("phoneNumber")&&<Error text={field("phoneNumber")!}/>}</label></div>
    ...
    <button disabled={isSubmitting || !mounted} type="submit" className="flex min-h-13 w-full items-center justify-center gap-2 rounded-full bg-emerald px-6 py-4 font-bold text-white transition hover:bg-[#063d35] disabled:opacity-60">{isSubmitting&&<LoaderCircle size={18} className="animate-spin"/>}{isSubmitting?"Registering securely…":"Complete registration"}</button>
...
function Error({text}:{text:string}){return <span role="alert" className="mt-1.5 block text-xs font-medium text-red-600 break-words leading-tight">{text}</span>}
