...
type EditableProgram = {
  ...
  registration_id_entry_key: string | null;
  full_name_entry_key: string | null;
...
export function ProgramEdit({ program }: { program: EditableProgram }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setPending(false);
    if (!response.ok) {
      setError(result.message ?? "Programme could not be updated.");
      return;
    }
    setOpen(false);
    router.refresh();
  ...
  return (
    <>
      <button
        ...
        className="flex min-h-10 items-center gap-2 rounded-full border border-emerald px-4 text-xs font-bold text-emerald transition-colors hover:bg-emerald/10"
      >
        <Pencil size={15} />
        Edit
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-[70] grid place-items-center overflow-y-auto bg-black/55 p-4">
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby={`edit-${program.id}`}
            className="my-4 w-full max-w-2xl rounded-2xl bg-white p-5 shadow-2xl sm:p-7"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  Programme identity and eligibility remain unchanged.
                </p>
              </div>
              <button
                onClick={() => setOpen(false)}
                aria-label="Close edit dialog"
                className="grid size-10 place-items-center rounded-full bg-cream hover:bg-ink/10 transition-colors"
              >
                <X size={18} />
              </button>
            </div>
            ...
            {isSwalath && (
              <div className="mt-4 flex items-center gap-2 rounded-xl border border-gold/30 bg-gold/10 p-3.5 text-xs font-bold text-ink">
                <Sparkles size={16} className="text-gold" />
                <span>
                  Swalath Campaign Mode: Google Form URL and Entry Keys are omitted. Configure campaign start and end dates below.
                </span>
              </div>
            )}
            ...
            <form onSubmit={submit} className="mt-6 space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="block text-sm font-bold">
                    ...
                    <input
                      ...
                      type="date"
                      defaultValue={program.campaign_start_date ?? "2026-08-22"}
                      required
                  ...
                  <label className="block text-sm font-bold">
                    ...
                    <input
                      ...
                      type="date"
                      defaultValue={program.campaign_end_date ?? "2026-09-05"}
                      required
                      className="mt-2 h-11 w-full rounded-xl border border-ink/15 px-3 font-semibold"
                    />
                  </label>
              ...
              <label className="block text-sm font-bold">
                ...
                <select
                  name="global_status"
                  defaultValue={program.global_status}
                  className="mt-2 h-11 w-full rounded-xl border border-ink/15 px-3"
                >
              ...
              <label className="block text-sm font-bold">
                ...
                <textarea
                  ...
                  maxLength={1000}
                  className="mt-2 min-h-28 w-full rounded-xl border border-ink/15 p-3"
                />
              </label>
              <label className="block text-sm font-bold">
                Rules (Malayalam)
                <textarea
                  name="rules"
                  defaultValue={program.rules ?? ""}
                  maxLength={5000}
                  className="mt-2 min-h-28 w-full rounded-xl border border-ink/15 p-3 font-malayalam"
                />
              </label>
